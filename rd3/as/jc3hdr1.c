...
GLOBAL LONG xydata = NO;         /*put out in X Y columns */
...
VOID jchdr1(
    ...
    if (stype >= 10)
        stype =  stype % 10;
    i = min(stype,3);      /* change if figure out stypes */
    if (sstype < 0)
        stype2 = "SPECTRUM";
    else
        stype2 = "TIME DRIVE";
...
LOCAL VOID slashes(
    ...
    if (!strnicmp(line,xydatac,lxy))
        {
        xydata = YES;
        return;
        }
	 if (!strnicmp(line,"$$",2)) return;	/* ignore comment */
