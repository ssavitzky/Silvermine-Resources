/*********************************************************************\
**
** SHIPPER		RD3 disk duplication program
**
**		This program reads a file of serial numbers and customer names
**		and creates a disk containing a customized copy of RD3 for
**		each one.
**
...
**            +-----------note required space
**			  V
**			-d dir		directory containing product to ship 	(RD3)
**			-p file		product to customize					(RD3.EXE)
**			-o drive	drive to output to						(B:)
**			-s string	serial number to start from
**			-n number	number of disks
**			file		input file name
**
**		The serial-number, customer-name, and Xor strings are
**		taken from #define's in the include file "version.h".
**
**		If the -n and -s options are given, shipper will not read an input
**		file, but instead produces products with sequential serial numbers
**		starting with the one given.  No attempt will be made in this
**		case to customize a customer name.  The serial number must end
...
**	900930 SS add -n option; get product from version.h
**
\*********************************************************************/

static char *pgmid="shipper 1.2\
Copyright (c) 1988,1990 Silvermine Resources, Inc.";
...
char *dirName  = PROGRAM;		/* product directory name */
char *prodName = ENV_VAR ".EXE";/* product to customize MUST BE UPPERCASE */
char *outDisk  = "B:";			/* output disk drive */

FILE *infile;					/* input database file */
char *infileName;				/* its name */
char *serialNum;				/* sequential serial number if non-zero */
int  count = 0;					/* count of disks */
int  testing = 0;
...
** nextSn()
**
**		increment the ascii serial number in nbuf.
**
\*********************************************************************/
...
void nextSn()
{
	...
	for (c = 1, i = strlen(nbuf) - 1; c && i >= 0; --i) {
		d = nbuf[i] + c;
		if (d >= '0' && d <= '9') {
			c = 0;
		} else if (d == '9' + 1) {
			...
			fprintf(stderr, "Serial number overflow: '%s'\n", nbuf);
			exit(1);
			c = 0;
		}
		nbuf[i] = d;
	...
	char **argv;
{
	...
	if (infileName && (count || serialNum)) {
		fprintf(stderr, "Cannot give both serial number and input file\n");
		exit(1);
	} else if (infileName) {
	...
	} else if (serialNum && !count) {
		fprintf(stderr, "Serial number given but no disk count.\n");
		exit(1);
	} else if (!serialNum) {
		fprintf(stderr, "No input file or serial-number given\n");
		exit(1);
	}
