...
#define DPTR_BLK(p) ((p)[2]+((long)(p)[3]<<8)+((long)(p)[4]<<16))
#define DPTR_NUM_(p,n) ((p)[0]=((n) & 255), (p)[1] = ((n) >> 8) & 255)
#define DPTR_BLK_(p,n) ((p)[2]=((n) & 255), (p)[3] = ((n) >> 8) & 255,\
						(p)[4]=((n) >> 16) & 255)

#define IPTR_SIZE 4
#define IPTR_NUM(p) ((short)(p)[0])
#define IPTR_BLK(p) ((p)[1]+((long)(p)[2]<<8)+((long)(p)[3]<<16))
...
typedef struct XFile {
	...
	char   *sec_buf;				/* -> current sector buffer */
...
global Dir dReadXDisk();		/* (drive, view) -> root	read disk	*/
